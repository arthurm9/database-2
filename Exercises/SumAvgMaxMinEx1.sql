...
-- 1 Informar o salario total de médicos que atuam em bauru com salario entre 3000 e 5000
...
WHERE
    cidade = 'Bauru'
        AND (salario >= 3000 AND salario <= 5000);

-- 2 Informar o media salarial de médicos que atuam em agudos com salario entre 1000 e 5000
SELECT 
...
WHERE
    cidade = 'Agudos'
        AND (salario >= 1000 AND salario <= 5000);

-- 3 Informar o maior salario entre cardiologistas, ortopedistas e pediatras que atuem em agudos
SELECT 
...
WHERE
    (especialidade = 'Cardiologista'
        OR especialidade = 'Ortopedista'
        OR especialidade = 'Pediatra')
        AND cidade = 'Agudos';
 

-- 4 Informar menor salario entre cardiologistas, ortopedistas e pediatras que atuem em agudos
...
WHERE
    (especialidade = 'Cardiologista'
        ...
        OR especialidade = 'Pediatra')
        AND cidade = 'Agudos'
        AND (dtadm >= '2000/01/01'
        AND dtadm <= '2015/12/31');

-- 5 Informar a quantidade total de medicos, a soma dos salarios, o salario medio, o maior e o menor salario
-- dos medicos cardiologistas, ortopedistas e pediatras
-- que foram contradados entre 2000 e 2015
-- que atuem em bauru, jau ou agudos
-- com faixa salarial entre 2999.99 e 5999.99
SELECT 
    COUNT(medico) 'QTD MEDICOS',
    SUM(salario) 'SOMA SALARIAL',
    CAST(AVG(salario) AS DECIMAL (8 , 2 )) 'MÉDIA SALARIAL',
    MAX(salario) 'MAIOR SALÁRIO',
    MIN(salario) 'MENOR SALÁRIO'
FROM
...
WHERE
    (especialidade = 'Cardiologista'
        ...
        AND (dtadm >= '2000/01/01'
        AND dtadm <= '2015/12/31')
        AND (cidade = 'Bauru' OR cidade = 'Jau'
        OR cidade = 'Agudos')
        AND (salario >= 2999.99
        AND salario <= 5999.99);
      
-- 6 Informar a quantidade total de medicos, a soma dos salarios, o salario medio, o maior e o menor salario
-- de medicos com IDMED entre 9 e 99
-- que atuem em bauru
...
FROM
    medicos
WHERE
    (idMed >= 9 AND idMed <= 99)
        AND cidade = 'Bauru'
        AND salario > 7000
        AND especialidade = 'Cardiologista'
        AND dtadm >= '2016/01/01';