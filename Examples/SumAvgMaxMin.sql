...
SELECT 
    COUNT(medico) 'Total de Médicos',
    SUM(salario) 'Total de Salario',
    CAST(AVG(salario) AS DECIMAL (8 , 2 )) 'Media Salarial',
...
WHERE
    (especialidade = 'pediatra'
        ...
        AND (cidade = 'bauru' OR cidade = 'lins')
        AND (salario > 3000 AND salario < 10000)
        AND (dtadm >= '2014/01/01'
        AND dtadm <= '2020/12/31');
